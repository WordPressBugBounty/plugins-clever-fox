...
function artisan_block_categories( $categories ) {
...
if ( version_compare( $GLOBALS['wp_version'], '5.8-alpha-1', '<' ) ) {
    add_filter( 'block_categories', 'artisan_block_categories', 10, 2 );
} else {
    add_filter( 'block_categories_all', 'artisan_block_categories', 10, 2 );
}

// Register Blocks
function artisan_wp_register_script( $block, $options = array() ) {
    register_block_type( 'info-box/' . $block,
        array_merge( array(
            'editor_script' => 'artisan_editor_script',
            'editor_style'  => 'artisan_editor_style',
            'script'        => 'artisan_script',
            'style'         => 'artisan_style',
        ), $options )
    );
}
...
// Enqueue assets
function artisan_enqueue_block_assets() {
    wp_enqueue_script( 'font-awesome-kit', CLEVERFOX_PLUGIN_URL . '/inc/artisan/block/assets/js/font-awesome-kit.js', array(), '1.0', true );
}
add_action( 'enqueue_block_assets', 'artisan_enqueue_block_assets' );

function artisan_register() {
    // Resister script in editor
	wp_register_script( 'artisan_editor_script', CLEVERFOX_PLUGIN_URL . 'inc/artisan/block/dist/editor.js', array( 'wp-blob', 'wp-block-editor', 'wp-blocks', 'wp-components', 'wp-compose', 'wp-data', 'wp-element', 'wp-html-entities', 'wp-i18n', 'wp-rich-text', 'font-awesome-kit' ), '1.0', false ); 
    wp_register_style( 'artisan_editor_style', CLEVERFOX_PLUGIN_URL . 'inc/artisan/block/dist/editor.css', array( 'wp-edit-blocks'), filemtime(CLEVERFOX_PLUGIN_DIR . 'inc/artisan/block/dist/editor.css') );

    // Register Blocks
    artisan_wp_register_script( 'infos', array( 'render_callback' => 'render_artisan_infos' ) );
    artisan_wp_register_script( 'info', array( 'render_callback' => 'render_artisan_info' ) );
}
add_action( 'init', 'artisan_register' );

// Generate Styles
class  ArtisanInfoStyleGenerator {
...
// Render Infos
function render_artisan_infos($attributes, $content){
    $cId = isset($attributes['cId']) ? esc_attr($attributes['cId']) : '';

    // Generate Styles
    $infosStyle = new  ArtisanInfoStyleGenerator();
...
// Render Info
function render_artisan_info( $attributes ) {
    extract( $attributes );
    ...
    // Generate Styles
    $infoStyle = new  ArtisanInfoStyleGenerator();
    $infoStyle::addStyle("#artisanInfo-$cId .artisanInfo .icon", array(
    ...
    // echo "<div class='av-column-".$columns['desktop']." ".$infoTypes."' id='artisanInfo-$cId'>
	...
	echo "<div class='av-column-" . esc_attr($columns['desktop']) . " " . esc_attr($infoTypes) . "' id='artisanInfo-" . esc_attr($cId) . "'>
        <aside class='widget widget-contact'>
            <div class='contact-area'>
