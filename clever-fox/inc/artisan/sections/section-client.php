<?php  
	...
	$client_contents		= get_theme_mod('client_contents',artisan_get_client_default());
	if($client_hs == '1'){
?>
	<!-- sponsors -->
		...
		<div class="row">				
			...
			<?php
				...
				foreach ( $client_contents as $client_item ) {
					$link = ! empty( $client_item->link ) ? apply_filters( 'artisan_pro_translate_single_string', $client_item->link, 'Client section' ) : '';
					$image = ! empty( $client_item->image_url ) ? apply_filters( 'artisan_pro_translate_single_string', $client_item->image_url, 'Client section' ) : '';
