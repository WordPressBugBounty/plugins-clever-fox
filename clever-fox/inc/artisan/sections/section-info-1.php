<?php  
	...
	$info		= get_theme_mod('info',artisan_get_info_default());	
	$info_column		= get_theme_mod('info_column','3');
	if($info_hs=='1'){
	$settings               = array('items'=>$info_column);
    wp_register_script('artisan-info',get_template_directory_uri().'/assets/js/homepage/info.js',array('jquery'));
	wp_localize_script('artisan-info','info_setting',$settings);
    wp_enqueue_script('artisan-info');
		if ( ! empty( $info ) ) {
			$info = json_decode( $info );
?>
<!-- Info section -->
<div class="info-section artisan-info">
    <div class="container">
